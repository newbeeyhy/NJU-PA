...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs) {
	...
	if ((sig_grs >> (23 + 3)) > 1 || exp < 0) {
		...
		// printf("\e[0;31mcase1\e[0m\n");
		while ((((sig_grs >> (23 + 3)) > 1) && exp < 0xff) || // * condition 1
			   (sig_grs > 0x4 && exp < 0)) {				  // * condition 2
			// todo: shift right, pay attention to sticky bit
			sig_grs = (sig_grs | ((sig_grs & 0x1) << 1)) >> 1;
            exp++;
		}
	...
	} else if (((sig_grs >> (23 + 3)) == 0) && exp > 0) { // * normalize toward left
		// printf("\e[0;31mcase2\e[0m\n");
	...
	} else if (exp == 0 && sig_grs >> (23 + 3) == 1) { // * two denormals result in a normal
		// printf("\e[0;31mcase3\e[0m\n");
		exp++;
	}

	if (!overflow) {
		// todo: round up and remove the GRS bits
		uint32_t last = (sig_grs >> 3) & 0x1;
        uint32_t grs = sig_grs & 0x7;
        sig_grs >>= 3;
		sig_grs += grs > 4 || (grs == 4 && last == 1);

        if (sig_grs >> 23 > 1) {
			// printf("\e[0;31mcase4\e[0m\n");
			sig_grs = (sig_grs | ((sig_grs & 0x1) << 1)) >> 1;
            exp++;
            if (exp == 0xff) {
                sig_grs = 0;
...
uint32_t internal_float_add(uint32_t b, uint32_t a) {
	...
	// printf("\e[0;31mexp_a: %x, exp_b: %x, shift: %x\e[0m\n", fa.exponent, fb.exponent, shift);

	sig_a <<= 3; // * guard, round, sticky
	sig_b <<= 3;
	while (shift > 0) {
		sig_a = (sig_a | ((sig_a & 0x1) << 1)) >> 1;
		shift--;
	}
